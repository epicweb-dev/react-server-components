...
Then we can add `isPending`, `nextLocation`, and `location` to our router
context. Compare `nextLocation` with `location` to see which part of the URL is
changing, and combine that with `isPending` so we only show pending UI while a
transition is actually in progress.
