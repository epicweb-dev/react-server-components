...
global one from `react`. We need to use one from `useTransition` instead so our
navigation updates are marked as transitions.
...
Then we can add `nextLocation` to our router context and compare it with
`location` to determine pending states for our UI.
