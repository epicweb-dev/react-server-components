...
1. The `public/index.html` is what we want loaded for browser document requests
   to our application.
2. The application is served by an express server that serves the static assets
   and the API endpoint.

This is not an node/express workshop, so we'll be really hand-holdy on things to
...
// because this file was loaded from /js/src/index.js, the browser will know to
// import this file from /js/src/app.js
...
As mentioned, this is not a workshop about express, but we're using it to serve
...
// this line tells express to serve the files in the public directory when a
...
Express has a special format for defining routes that can be used to match
