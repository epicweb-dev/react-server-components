...
something like
`file:///Users/kentcdodds/code/epicweb-dev/react-server-components/playground/src/ship-search.js`.
...
// moduleBasePath is something like `file:///Users/kentcdodds/code/epicweb-dev/react-server-components/playground/src`
const { pipe } = renderToPipeableStream(payload, moduleBasePath)
```

With the full file path removed, we're left with the path to the module relative
to our `src` directory. Our express server is configured to serve any file in
`src` via `/js/src`.
